...
export default function ProductUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = productRecord
      ? { ...initialValues, ...productRecord, tags: linkedTags, factory }
      : initialValues;
    setName(cleanValues.name);
    setMsrp(cleanValues.msrp);
    setPrice(cleanValues.price);
    setSku(cleanValues.sku);
    setDescription(cleanValues.description);
    setAvailable(cleanValues.available);
    setMedia(cleanValues.media ?? []);
    setItem_weight_net(cleanValues.item_weight_net);
    setItem_weight_gross(cleanValues.item_weight_gross);
    setItem_size_width(cleanValues.item_size_width);
    setItem_size_depth(cleanValues.item_size_depth);
    setItem_size_height(cleanValues.item_size_height);
    setPackage_weight_net(cleanValues.package_weight_net);
    setPackage_weight_gross(cleanValues.package_weight_gross);
    setPackage_size_width(cleanValues.package_size_width);
    setPackage_size_depth(cleanValues.package_size_depth);
    setPackage_size_height(cleanValues.package_size_height);
    setItems_per_package(cleanValues.items_per_package);
    setTags(cleanValues.tags ?? []);
    setCurrentTagsValue(undefined);
    setCurrentTagsDisplayValue("");
    setFactory(cleanValues.factory);
  ...
  React.useEffect(() => {
    const queryData = async () => {
      ...
      const factoryRecord = record ? await record.factory : undefined;
      setFactory(factoryRecord);
    };
    queryData();
  }, [idProp, productModelProp]);
  React.useEffect(resetStateValues, [productRecord, linkedTags, factory]);
  ...
  return (
          ...
          promises.push(
            DataStore.save(
              Product.copyOf(productRecord, (updated) => {
                ...
                if (!modelFieldsToSave.factory) {
                  updated.factory_id = undefined;
                }
              })
            )
          );
    ...
    >
      ...
      >
        {productRecord && (
          <StorageManager
            defaultFiles={productRecord.media.map((key) => ({ key }))}
            onUploadSuccess={({ key }) => {
              setMedia((prev) => {
                let value = [...prev, key];
                if (onChange) {
