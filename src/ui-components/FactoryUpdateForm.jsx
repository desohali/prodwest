...
export default function FactoryUpdateForm(props) {
  const {
    id: idProp,
    factory: factoryModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = factoryRecord
      ? { ...initialValues, ...factoryRecord, products: linkedProducts }
      : initialValues;
    setName(cleanValues.name);
    setUrl(cleanValues.url);
    setOrder_contacts(cleanValues.order_contacts ?? []);
  ...
  const [factoryRecord, setFactoryRecord] = React.useState(factoryModelProp);
  const [linkedProducts, setLinkedProducts] = React.useState([]);
  const canUnlinkProducts = true;
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Factory, idProp)
        : factoryModelProp;
      setFactoryRecord(record);
      const linkedProducts = record ? await record.products.toArray() : [];
      setLinkedProducts(linkedProducts);
    };
    queryData();
  }, [idProp, factoryModelProp]);
  React.useEffect(resetStateValues, [factoryRecord, linkedProducts]);
  ...
  return (
    <Grid
        ...
        try {
          ...
          productsToUnLink.forEach((original) => {
            if (!canUnlinkProducts) {
              throw Error(
                `Product ${original.id} cannot be unlinked from Factory because undefined is a required field.`
              );
            }
            promises.push(
              DataStore.save(
                Product.copyOf(original, (updated) => {
                  updated.factory = null;
                })
              )
            );
          });
          ...
          promises.push(
            DataStore.save(
              Factory.copyOf(factoryRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "FactoryUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || factoryModelProp)}
